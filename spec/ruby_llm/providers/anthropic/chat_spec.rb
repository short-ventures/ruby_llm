...
RSpec.describe RubyLLM::Providers::Anthropic::Chat do
  describe '.render_payload' do
    let(:model) { instance_double(RubyLLM::Model::Info, id: 'claude-sonnet-4-6', max_tokens: nil) }
    ...
    it 'renders adaptive thinking and low effort guidance for supported models' do
      user_message = RubyLLM::Message.new(role: :user, content: 'Hello there')
      ...
      expect(payload[:output_config]).to eq({ effort: 'low' })
    end
    ...
    it 'does not enable adaptive thinking on unsupported models' do
      model_45 = instance_double(RubyLLM::Model::Info, id: 'claude-sonnet-4-5', max_tokens: nil)
      user_message = RubyLLM::Message.new(role: :user, content: 'Hello there')
      thinking = RubyLLM::Thinking::Config.new(effort: :low)
    ...
    it 'merges schema output_config with low effort guidance' do
      schema = {
        name: 'response',
        schema: { type: 'object', properties: { name: { type: 'string' } } },
        strict: true
